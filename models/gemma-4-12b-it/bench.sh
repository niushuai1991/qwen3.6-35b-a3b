#!/usr/bin/env bash
# bench.sh - Gemma-4-12B-it 推理速度基准测试
#
...
#
# NOTE: chat_template_kwargs 字段按模型族调整（避免 bench 时意外触发思考模式）：
#   - Qwen3.x: {"chat_template_kwargs": {"enable_thinking": false}}
#   - Gemma-4 / Llama 3.x: 不需要该字段（默认不思考）
...
# 显存（仅当容器在跑）
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^gemma-4-12b-it$'; then
    echo
    echo "## VRAM"
    echo
    echo '```'
    docker exec gemma-4-12b-it nvidia-smi \
