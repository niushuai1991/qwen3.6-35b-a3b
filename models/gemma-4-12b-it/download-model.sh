#!/usr/bin/env bash
# download-model.sh - 下载 Gemma-4-12B-it 的两个 gguf 文件
#
# 优先级：modelscope（国内 CDN 快） → HF 镜像兜底
# 已存在的文件自动跳过，天然支持断点续传
#
# 可覆盖的环境变量（默认值与 .env 保持一致）：
#   TARGET_DIR   /home/ns/models/gemma-4-12b-it
#   MODEL_FILE   gemma-4-12b-it-Q4_K_M.gguf
#   MMPROJ_FILE  mmproj-F16.gguf
#   HF_REPO      unsloth/gemma-4-12b-it-GGUF
#   HF_ENDPOINT  https://hf-mirror.com
#   MS_REPO      unsloth/gemma-4-12b-it-GGUF（空：跳过 modelscope）
#
# 用法：
#   ./download-model.sh                         # 下载全部
#   TARGET_DIR=/data/xxx ./download-model.sh    # 改路径
#   MODEL_FILE=xxx-Q5_K_M.gguf ./download-model.sh  # 换量化

set -euo pipefail

TARGET_DIR="${TARGET_DIR:-/home/ns/models/gemma-4-12b-it}"
MODEL_FILE="${MODEL_FILE:-gemma-4-12b-it-Q4_K_M.gguf}"
MMPROJ_FILE="${MMPROJ_FILE:-mmproj-F16.gguf}"
HF_REPO="${HF_REPO:-unsloth/gemma-4-12b-it-GGUF}"
HF_ENDPOINT="${HF_ENDPOINT:-https://hf-mirror.com}"
MS_REPO="${MS_REPO:-unsloth/gemma-4-12b-it-GGUF}"
